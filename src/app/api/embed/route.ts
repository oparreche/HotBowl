...
export async function GET(request: NextRequest) {
  ...
  const js = `(() => {
    ...
    c.style.height='520px'; c.style.boxShadow='0 8px 24px rgba(0,0,0,0.24)';
    ...
    function handleMessage(e){ try { var data = e && e.data; if (data && data.type==='HB_EMBED' && data.action==='REMOVE') { if (c && c.parentNode) c.parentNode.removeChild(c); window.removeEventListener('message', handleMessage); } } catch(_){} }
